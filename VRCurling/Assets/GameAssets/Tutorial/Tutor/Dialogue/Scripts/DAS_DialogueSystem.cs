...
public class DAS_DialogueSystem : MonoBehaviour
{
    ...
    public int currentDiagIndex = 0; // 添加這一行
    ...
    {
        if (!active && !dialogueController.dialogueInCourse) // 這個條件檢查確保在開始對話之前沒有其他對話正在進行。
        {
            active = true;
    ...
    public void endDialogue()
    {
        ...
        {
            tutorController.OnTutorialEnd.Invoke();
        }
    ...
    public void callSpecificDialogue(DAS_Dialogue dialogue)
    {
        ...
        for (int i = 0; i < dialogues.Length; i++)
        {
            ...
            {
                diagIndex = i;
                break; // 找到匹配的對話後立即退出循環
            }
        }
        if (diagIndex != -1) // 確保找到了有效的索引
        {
            currentPhraseIndex = 0;
            currentDialogueIndex = diagIndex;
            dialogueBox.showUpAnimation();

            if (currentDialogueIndex == 8 ||currentDialogueIndex == 14 )
            {
                
                StartCoroutine(WaitAndContinue(7));
