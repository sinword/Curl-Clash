...
Shader "Lite Effect/Particles/URP/Lite Particle Add"
{
	Properties
	{
...
CBUFFER_END
				...
				half4 frag(v2f i) : SV_Target
				{
					...
					_CameraFadeParams.y = 1.0f / (_CameraFarFadeDistance);
					i.color.a *= CameraFade(_CameraFadeParams.x,_CameraFadeParams.y, i.projPos);
					#endif
					...
					float4 ca = lerp(float4(1,1,1,1) , SAMPLE_TEXTURE2D(_CutTex,sampler_CutTex, i.uv_CutOut).aaaa,_isCut);
					c *= ca;

					float4 Light = smoothstep( 1-_light,1,c);
